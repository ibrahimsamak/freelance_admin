...
@Component({
  selector: 'app-notification-in',
  templateUrl: './notification-in.component.html',
  styleUrls: ['./notification-in.component.scss']
})
export class NotificationInComponent implements OnInit {
  ...
  getNotifications(page, limit, filter) {
    this.helper.getNotifications2(page, limit, filter).subscribe((x) => {
  ...
  public onPageChange(pageNum: number): void {
    this.page = pageNum - 1;
    this.helper
      .getNotifications2(this.page, this.limit, this.searchObject)
      .subscribe((x) => {
        let arr = x[appConstant.ITEMS] as any[];
        // this.collectionCount = x["pagination"]["totalElements"];
        this.notifications = arr;
  ...
  excel() {
    ...
    this.helper
      .getExcelNotifications2(this.searchObject)
      .subscribe((res_data) => {
